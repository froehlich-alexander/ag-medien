...
interface ImportDialogState {
    ...
    // true if files input was updated but the update not yet represented in DOM
...
class ImportDialog extends Component<ImportDialogProps, ImportDialogState> {
    ...
    private badFileToast = createRef<HTMLDivElement>();
    private invalidColorSchemesToast = createRef<HTMLDivElement>();
    // a counter to generate unique ids
    private nextColorSchemeId = 0
                        ...
                        <div className={"modal-body"}>
                            <Form action="javascript:void(0)"
                                ...
                                <div className={"input-group"}>
                                    ...
                                    <div className="toast-container position-fixed end-0 bottom-0 p-3">
                                        {/*Bad Files Toast*/}
                                        <div className="toast"
                                             role="alert"
        ...
        Promise.all(Array.from(event.target.files!).map(file => file.text()
            .then(text => new Promise<ColorSchemesFile>((resolve, reject) => {
                ...
                .then(v => v.colorSchemes.map(v => ColorScheme.fromJSON(v).withUpdate({
                    current: false,
                    preDefined: false,
                    id: (++this.nextColorSchemeId).toString(),
                })))
                .then(value => {
            ...
            .then(colorSchemes => Promise.allSettled(
                colorSchemes.map(cs => new Promise<ColorSchemeFileInfo>((resolve, reject) => {
                    ...
                    if (this.props.allColorSchemes.some(v => cs.colorScheme.equalsFragment(v))||
                    this.state.colorSchemesFromFile.some(v=>cs.colorScheme.equalsFragment(v))) {
    ...
    public componentDidMount(): void {
        ...
        new Toast(this.invalidColorSchemesToast.current!);
    }
